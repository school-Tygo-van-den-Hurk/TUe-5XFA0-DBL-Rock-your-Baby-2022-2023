 ...
 * - loop(void)
 *   - blink(void) 
 *   - improveFrequentcy(void)
 *   - improveAmplitude(void)
 ...
 * 
 * @pre {@code array != null && amountOfMeasurements != 0 && heartbeatSensor != null}
 * @return the mode of n measurements, each with a delay of m miliseconds.
 */
int currentHeartbeat() {
  int current_bmp = heartbeatSensor.next();
  Serial.println("         \\==> current_bpm := " + current_bmp);
  return current_bmp;
}
